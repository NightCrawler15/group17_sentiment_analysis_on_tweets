...
class Tweets(MRJob):
    ...
    def mapper(self, _, line):
        ...
        txt = Tweets._filter_text(str(column[1]))
        pos = 0
        neg = 0
        total = 0
        for word in txt.split():
            clean_wrd = Tweets._clean_word(word)
            score = Tweets._eval_word(clean_wrd)
            if score >= 0:
                pos = pos + score
            else:
                neg = neg + score
            total += 1
        if round(pos/total, 2) > round(abs(neg/total), 2):
            yield "Positive", 1
        elif  round(pos/total, 2) <  round(abs(neg/total), 2):
            yield "Negative", 1
        else:
            yield "Neutral", 1
